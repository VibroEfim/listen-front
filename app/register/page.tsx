...
export default function Upload() {
  ...
  const submitForm = async () => {
    let data = await api.register(name, password);

    if (data.status == 200) {
      NotificationService.addNotification("Регистрация успешна, вот-вот обновлю страницу!", NotificationService.Types.SUCCESS, 3000);
  ...
  return (
    <Box margin={6}>
      <Box width={430}>
        <Typography color={"white"} variant="h4">Регистрация</Typography>
        ...
        <Box width={400} textAlign={"center"} marginTop={1}>
          <Link className={common.clickable} href="/login">У меня уже есть аккаунт :)</Link>
        </Box>
      </Box>
    </Box>
  );
